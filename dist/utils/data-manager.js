...
function () {
      ...
      if (_this.applyFilters) {
        _this.columns.filter(function (columnDef) {
          ...
          if (columnDef.customFilterAndSearch) {
            ...
            if (lookup) {
              _this.filteredData = _this.filteredData.filter(function (row) {
                return !tableData.filterValue || tableData.filterValue.length === 0 || tableData.filterValue.indexOf(row[field] !== undefined && row[field].toString()) > -1;
              });
  ...
  (0, _createClass2["default"])(DataManager, [{
    ...
    value: function changeByDrag(result) {
      ...
      } else if (result.destination.droppableId === "headers" && result.source.droppableId === "headers") {// Column reordering
      } else {
        return;
      }
    ...
    value: function findGroupByGroupPath(renderData, path) {
      ...
      var node = path.reduce(function (result, current) {
        ...
        var group = result.groups.find(function (a) {
          return a.value === current;
        });
        return group;
      }, data);
      return node;
    ...
    value: function groupData() {
      ...
      var subData = tmpData.reduce(function (result, current) {
        var object = result;
        object = groups.reduce(function (o, colDef) {
          var value = current[colDef.field] || _this5.byString(current, colDef.field);

          var group = o.groups.find(function (g) {
    ...
    value: function treefyData() {
      ...
      var addRow = function addRow(rowData) {
        ...
        if (parent) {
          parent.tableData.childRows = parent.tableData.childRows || []; // parent.tableData.isTreeExpanded = false;
